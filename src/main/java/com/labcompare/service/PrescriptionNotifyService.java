...
public class PrescriptionNotifyService {
    ...
    // Both team emails go in CC on every notification.
    // The TO is always the patient/user email passed at call time.
    @Value("${app.prescription.team-cc-1}")
    private String teamCc1;

    @Value("${app.prescription.team-cc-2}")
    private String teamCc2;
    ...
    public void sendPrescriptionToTeam(String userName, String userPhone,
                                        String userEmail, MultipartFile file) throws Exception {
        String timeNow   = now();
        ...
        Map<String, Object> payload = buildPayload(
            ...
            subject, html, userEmail,
            List.of(Map.of("filename", filename, "content", b64, "content_type", mime))
        );
    ...
    public void sendPaymentSuccessToTeam(String userName,
        ...
        try {
            ...
            Map<String, Object> payload = buildPayload(
                ...
                subject, html, userEmail,
                List.of(Map.of("filename", attachName, "content", attachB64, "content_type", "text/plain"))
            );
    ...
    // TO:  patient/user email (whoever filled the form)
    // CC:  drmedicalfoundation@gmail.com + saddamkhan212201@gmail.com (always)
    // If userEmail is blank, fall back to CC-1 as primary recipient.
    private Map<String, Object> buildPayload(String from, String subject, String html,
                                              String userEmail,
                                              List<Map<String, String>> attachments) {
        Map<String, Object> p = new HashMap<>();
        p.put("from",    from);

        boolean hasUserEmail = userEmail != null && !userEmail.isBlank();
        p.put("to", hasUserEmail ? List.of(userEmail) : List.of(teamCc1));
        p.put("cc", hasUserEmail ? List.of(teamCc1, teamCc2) : List.of(teamCc2));

        p.put("subject", subject);
        p.put("html",    html);
        if (attachments != null && !attachments.isEmpty()) p.put("attachments", attachments);
        return p;
    }

...
public class PriceService {
    ...
    public List<PriceDTO> searchPrices(Long testId, String city) {
        ...
        if (hasTestId && hasCity) {
            results = priceRepository
                .findByTestIdAndLab_CityContainingIgnoreCaseOrderByPriceAsc(testId, city);
        } else if (hasTestId) {
            results = priceRepository
                .findByTestIdOrderByPriceAsc(testId);
        } else if (hasCity) {
            results = priceRepository
                .findByLab_CityContainingIgnoreCaseOrderByPriceAsc(city);
        } else {
